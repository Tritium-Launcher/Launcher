...
import io.github.tritium_launcher.api.io.*
...
class ModBrowserDockPanelProvider : DockPanelProvider {
...
class ModBrowserSidePanel(
    ...
    private val provider: ModBrowserDockPanelProvider? = null
) : QWidget(), Persistable {

    override val persistKey = "mod_browser"
    override val flushPolicy = FlushPolicy.Periodic
    ...
    override fun captureState() = buildJsonObject {
        put("lastSearchText", searchField.text.trim().takeIf { it.isNotBlank() })
    }

    override fun restoreState(state: JsonObject) {
        state["lastSearchText"]?.jsonPrimitive?.contentOrNull?.let {
            searchField.text = it
            lastQueryText = it
        }
    }
    ...
    init {
        ...
        searchField.textChanged.connect { _: String? -> markDirty() }
    ...
    private suspend fun downloadAndCacheIcon(
        ...
        if (subdir != null && sourceId != null) {
            ...
            if (cached != null) {
                CacheMngr.touch(cacheFile(subdir, sourceId, iconUrl))
                runOnGuiThread {
        ...
        val bytes = runCatching {
            if (iconUrl.startsWith("state:image/svg+xml,")) {
                iconUrl.removePrefix("state:image/svg+xml,").toByteArray(Charsets.UTF_8)
            } else {
        ...
        if (subdir != null && sourceId != null) {
            ...
            CacheMngr.evictIfNeeded(mbCacheDir, subdir)
        }
        runOnGuiThread {
                ...
                queued.map { queuedMod ->
                    async {
                        downloadSemaphore.withPermit {
                            try {
                                val resolved = resolveInstallDownload(
                                    context,
                                    source,
                                    queuedMod.projectId,
                                    queuedMod.versionId,
    ...
    private fun startDownloadsWatcher() {
        ...
        downloadsWatcher = downloadsDir.watch(
            ...
            options = VWatchOptions(
                kinds = listOf(StandardWatchEventKinds.ENTRY_CREATE),
            ),
            ctx = Dispatchers.IO
        )
    }
