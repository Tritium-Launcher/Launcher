...
import io.github.tritium_launcher.api.io.VPath.Companion.parse
...
class VPathWatcher internal constructor(
    private val service: WatchService? = null,
    private val job: Job,
    private val registeredKeys: Map<WatchKey, VPath>? = null
): Closeable {
    override fun close() {
        try {
            job.cancel()
    ...
    fun registeredDirectories(): List<VPath> = registeredKeys?.values?.toList() ?: emptyList()
}
...
fun VPath.watch(
    ...
    val job = scope.launch {
        watchAsFlow(options).collect { callback(it) }
    }

    return VPathWatcher(job = job)
}
                    ...
                    if (options.recursive && kind == ENTRY_CREATE) {
                        ...
                        if (ctx != null) {
                            ...
                            try {
                                ...
                                if (Files.isDirectory(childJvm)) {
                                    Files.walkFileTree(childJvm, object : SimpleFileVisitor<Path>() {
                                        override fun visitFile(
                                            file: Path,
                                            attrs: BasicFileAttributes
                                        ): FileVisitResult {
                                            sendEvent(VWatchEvent(VWatchEvent.Kind.Create, parse(file.toString())))
                                            return FileVisitResult.CONTINUE
                                        }
