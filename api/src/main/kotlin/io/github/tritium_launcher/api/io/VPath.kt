...
package io.github.tritium_launcher.api.io
...
data class VPath(
    ...
    internal val logger = LoggerFactory.getLogger(VPath::class.java)
    ...
    /**
     * Move this file/directory to [target].
     * Delegates to [Files.move].
     * Creates parent directories of [target] before moving.
     * @return true if the move succeeded, false otherwise.
     */
    fun moveTo(target: VPath, vararg options: CopyOption): Boolean = try {
        val targetPath = target.toJPath()
        targetPath.parent?.let { Files.createDirectories(it) }
        Files.move(toJPath(), targetPath, *options)
        true
    } catch (e: Exception) {
        logger.warn("Exception moving path '$this' to '$target'", e)
        false
    }

    /**
     * Move this file to the OS recycling bin / trash.
     * Uses [Desktop.moveToTrash]; falls back to [delete] if trash is not supported.
    ...
    fun readText(charset: Charset = Charsets.UTF_8): String = String(Files.readAllBytes(toJPath()), charset)
    ...
    companion object {
        ...
        val userHome: VPath get() = VPath.get(getProperty("user.home"))
        /** Checks if this [String] matches any of the provided strings */
        private fun String.matches(vararg strings: String): Boolean = strings.any { this == it }

        /**
         * Returns [File] from ~/
         */
        fun fromHome(vararg child: String): VPath {
            return get(getProperty("user.home"), *child)
        }
